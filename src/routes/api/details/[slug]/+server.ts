...
export const GET = (async ({ fetch, params, locals }) => {
  ...
  return getOne(fetch, `${env.API_URL}/api/details`, params.slug);
...
export const PUT = (async ({ fetch, request, params, locals }) => {
  ...
  return put(fetch, `${env.API_URL}/api/details`, params.slug, await request.text());
}) satisfies RequestHandler;
...
export const DELETE = (async ({ fetch, params, locals }) => {
  ...
  return del(fetch, `${env.API_URL}/api/details`, params.slug);
}) satisfies RequestHandler;
